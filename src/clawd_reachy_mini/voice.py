...
def match_wake_phrase(text: str, name: str = "gizmo") -> str | None:
    ...
    """
    lowered = text.lower()
    # Name-first address ("Gizmo, install the updates") — exact alias at
    # utterance start ONLY, no soundex/fuzz, and a command must follow:
    # Whisper's hotword bias turns short room noises into bare "Gizmo"
    # transcripts, so the name alone does not wake (summoning with no
    # request still works via "hey gizmo").
    first = re.match(r"([a-z']+)[\s,.!?-]*", lowered)
    if first and (first.group(1) in WAKE_NAME_ALIASES or first.group(1) == name):
        remainder = lowered[first.end():].strip(" ,.!?")
        if remainder:
            return remainder
